...
if (isset($_GET['q']) && $_GET['q'] == 'addquiz') {
  $name = mysqli_real_escape_string($con, trim(strtolower($_POST['name'])));
  $total = intval($_POST['total']);
  $sahi = intval($_POST['right']);
  ...
  $query = "SELECT * FROM quiz WHERE LOWER(TRIM(title)) = '$name'";
  $result = mysqli_query($con, $query) or die('Error checking duplicate title');
  if (mysqli_num_rows($result) > 0) {
      echo "<script>alert('A quiz with this title already exists. Please choose a different title.');</script>";
      echo "<script>window.location.href='dash.php?q=4';</script>";
      exit();
  }
    // Insert the new quiz
    $id = uniqid();
    $query = "INSERT INTO quiz (eid, title, sahi, wrong, total, time, tag, description, date) 
              VALUES ('$id', '$name', '$sahi', '$wrong', '$total', '$time', '$tag', '$desc', NOW())";
